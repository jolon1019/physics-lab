...
/* ============ 场景公共元素 ============ */
function drawSky() {
  ctx.fillStyle = grads('#dfe9f5', '#f6f1e6'); ctx.fillRect(0, 0, CW, CH)
}
function drawGround() {
  ctx.fillStyle = '#cbb98f'; ctx.fillRect(0, groundY, CW, CH - groundY)
  ctx.fillStyle = 'rgba(0,0,0,0.15)'; ctx.fillRect(0, groundY, CW, 4)
  ctx.fillStyle = 'rgba(90,120,60,0.35)'
  for (let i = 0; i < Math.ceil(CW / 28); i++) {
    const x = (i * 53) % CW
    ctx.fillRect(x, groundY + 10 + (i % 3) * 6, 2, 6)
  }
}
