...
import { routeAllWires } from '../circuit/wireRouter'
  ...
  getters: {
    ...
    animatedBranches() {
      ...
      for (const b of sol.branches) {
        ...
        let pts
        if (kind === 'wire') {
          // 导线：使用避让布线结果（绕开元件/其它导线），保持与画布一致的折线
          // 注意：支路 id 为 `w:${wireId}`，而 wireRoutes 以原始 wireId 为键
          const wireId = b.id.startsWith('w:') ? b.id.slice(2) : b.id
          const rt = this.wireRoutes.get(wireId)
          if (rt && rt.length >= 2) {
            pts = rt
          } else {
