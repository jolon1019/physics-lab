// CDP 验证：启动 vite + headless Chrome，加载通电线圈实验（含换向器·无电源/电刷/轴承座版）
// 1) 检查运行期报错、canvas 挂载、通电按钮(.btn 含"通电")与手动换向按钮(.btn-flip)存在
// 2) 通电后模拟手动换向 → 线圈持续旋转(travel>360)
// 3) 断电 → 按钮切回"通电"，电流停（电子流速度为 0）
...
const SIM_BODY = `
  ...
  window.__simTimers[tag] = setInterval(function(){
    const powerBtn = [...document.querySelectorAll('.lab-actions .btn')].find(b => b.textContent.includes('通电'));
    if (powerBtn && !sim.played) { powerBtn.click(); sim.played = true; }
    ...
    try {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        const chk = await cdp('Runtime.evaluate', {
          expression: `!!document.querySelector('.btn-flip') && !!document.querySelector('canvas')`,
          returnByValue: true
        })
      ...
      const diag0 = await cdp('Runtime.evaluate', {
        expression: `(function(){
          ...
          const power = [...document.querySelectorAll('.lab-actions .btn')].find(b=>b.textContent.includes('通电'));
          return { hasCanvas: !!c, canvasW: c?c.width:0,
                   hasPowerBtn: !!power, hasFlipBtn: !!document.querySelector('.btn-flip'),
                   hasCommLabel: !!([...document.querySelectorAll('.coil-3d')].length) };
        })()`,
        returnByValue: true
      })
      console.log('DIAG0 ' + JSON.stringify(diag0.result.value))
      if (!diag0.result.value.hasCanvas) { errors.push('canvas 未挂载'); exitCode = 1 }
      if (!diag0.result.value.hasPowerBtn) { errors.push('通电按钮缺失'); exitCode = 1 }
      if (!diag0.result.value.hasFlipBtn) { errors.push('手动换向按钮缺失'); exitCode = 1 }
      ...
      console.log('SIM_A(通电·手动换向) ' + JSON.stringify({ travel: Math.round(a.travel), clicks: a.clicks, recent: Math.round(recentA) }))
      if (!(a.clicks >= 1 && recentA > 40)) {
        errors.push('通电后手动换向未持续旋转 (clicks=' + a.clicks + ', recent=' + Math.round(recentA) + ')')
        exitCode = 1
      }
      ...
      await cdp('Runtime.evaluate', {
        expression: `(function(){
          const pb = [...document.querySelectorAll('.lab-actions .btn')].find(b=>b.textContent.includes('断电'));
          if (pb) pb.click();
        })()`,
        returnByValue: true
      })
      await sleep(400) // 等待 Vue 重渲染按钮文本
      const diagB = await cdp('Runtime.evaluate', {
        expression: `(function(){
          const powerOff = [...document.querySelectorAll('.lab-actions .btn')].some(b=>b.textContent.includes('通电'));
          return { powerOff: powerOff };
        })()`,
        returnByValue: true
      })
      console.log('DIAG_B ' + JSON.stringify(diagB.result.value))
      if (!diagB.result.value.powerOff) { errors.push('断电失败（按钮未切回通电）'); exitCode = 1 }
      ...
      try {
        ...
        if (shot && shot.result && shot.result.data) {
          fs.writeFileSync('.shot_coil7.png', Buffer.from(shot.result.data, 'base64'))
          console.log('SCREEN .shot_coil7.png')
