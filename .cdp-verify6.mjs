// CDP 验证：启动 vite + headless Chrome，加载通电线圈实验（简化模型：无电源/电刷/换向器）
// 1) 检查运行期报错、canvas 挂载、通电按钮(.btn-power)存在
// 2) 点击「⚡ 通电」→ 线圈通电摆动；自动在发光时点「⇄ 手动换向」→ 维持持续旋转(travel>360)
...
const SIM_BODY = `
  ...
  window.__simTimers[tag] = setInterval(function(){
    const playBtn = [...document.querySelectorAll('.btn')].find(b=>b.textContent.includes('通电'));
    ...
    try {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        const chk = await cdp('Runtime.evaluate', {
          expression: `!!document.querySelector('.btn-power') && !!document.querySelector('canvas')`,
          returnByValue: true
        })
      ...
      const diag0 = await cdp('Runtime.evaluate', {
        expression: `(function(){
          ...
          const pb = document.querySelector('.btn-power');
          return { hasCanvas: !!c, canvasW: c?c.width:0,
                   hasPowerBtn: !!pb, powerText: pb?pb.textContent.trim():'' };
        })()`,
        returnByValue: true
      })
      ...
      if (!(a.clicks >= 1 && recentA > 40)) {
        errors.push('通电后手动换向未维持旋转 (clicks=' + a.clicks + ', recent=' + Math.round(recentA) + ')')
        exitCode = 1
      }

      // 阶段 B：断电 → 电流/受力箭头隐藏、线圈停转（点击后等一拍再读，避免读到旧 DOM）
      await cdp('Runtime.evaluate', {
        expression: `(function(){ const pb = document.querySelector('.btn-power'); if (pb) pb.click(); })()`,
        returnByValue: true
      })
      await sleep(400)
      const diagB = await cdp('Runtime.evaluate', {
        expression: `(function(){
          const pb = document.querySelector('.btn-power');
          return { powerOff: pb ? pb.textContent.includes('通电') : false };
        })()`,
        returnByValue: true
      })
      ...
      try {
        ...
        if (shot && shot.result && shot.result.data) {
          fs.writeFileSync('.shot_coil.png', Buffer.from(shot.result.data, 'base64'))
          console.log('SCREEN .shot_coil.png')
